...
class Pika:
    ...
    def focus_notepad(self):
        """Focus on the existing Notepad window"""
        try:
            # Try to bring Notepad to the front
            ...
            # If pygetwindow is not available, use Alt+Tab to cycle to Notepad
    ...
    def write_to_notepad(self, text):
        ...
        if "write " in text:
            ...
            # Type the text with a space and newline for better formatting
    ...
    def backspace_notepad(self):
        """Delete the last word in Notepad"""
        if not self.is_notepad_open():
        ...
        # Use Ctrl+Shift+Left to select the last word, then delete it
